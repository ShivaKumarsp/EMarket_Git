...
@Component({
  selector: 'app-send-hub-to-hub',
  templateUrl: './send-hub-to-hub.component.html',
  styleUrls: ['./send-hub-to-hub.component.css']
})
export class SendHubToHubComponent implements OnInit {
...
let url='Send_Hub_to_Hub/get_data/';
...
"language_id":1
  ...
  let url='Send_Hub_to_Hub/get_batch_data_details/';
...
pickupParcel(aa:any) {
this.hub_to_hub_array=[];

  this.pickup_delivery_list_details.forEach((ss:any)=>
  {
    if(aa.batch_id==ss.batch_id)
    {
      this.hub_to_hub_array.push({consignment_id:ss.consignment_id})
    }
  })
...
Swal.fire({
    ...
    if (result.isConfirmed) {
        ...
        var data = {
            ...
            "hub_to_hub_array":this.hub_to_hub_array,
            "language_id": 1
        }
        let url='Send_Hub_to_Hub/update_pickup_delivery/'
        this.allapi.PostData(url, data).subscribe(promise=> {
...
deliverParcel(aa:any) {
  ...
  Swal.fire({
      ...
      if (result.isConfirmed) {
          ...
          let url='Send_Hub_to_Hub/update_drop_delivery/'
          this.allapi.PostData(url, data).subscribe(promise=> {
